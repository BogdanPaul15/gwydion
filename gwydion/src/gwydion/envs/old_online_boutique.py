...
from gwydion.src.gwydion.envs.old_deployment import get_max_cpu, get_max_mem, get_max_traffic, \
    get_online_boutique_deployment_list
from gwydion.src.gwydion.envs.util import save_to_csv, get_num_pods, get_cost_reward, \
    ...
    def simulation_update(self):
        if self.current_step == 1:
            ...
            for i in range(self.num_apps):
                data = data.loc[self.df[DEPLOYMENTS[i] + '_num_pods'] == pods[i]]
                data = data.loc[data['diff-' + DEPLOYMENTS[i]] == diff[i]]

                ### New Traffic ###
                new_traffic = self.traffic.pop(0)
                #       data = data.loc[data['frontend_traffic_in'] == new_traffic]
        ...
        with file:
            ...
            writer.writerow(
                {'date': date,
                 ...
                 'recommendationservice_cpu_usage': int("{}".format(obs[1])),
                 'recommendationservice_mem_usage': int("{}".format(obs[2])),
                 ...
                 'productcatalogservice_num_pods': int("{}".format(obs[3])),
                 #           'productcatalogservice_desired_replicas': int("{}".format(obs[3])),
                 'productcatalogservice_cpu_usage': int("{}".format(obs[4])),
                 'productcatalogservice_mem_usage': int("{}".format(obs[5])),
                 ...
                 'cartservice_num_pods': int("{}".format(obs[6])),
                 #        'cartservice_desired_replicas': int("{}".format(obs[15])),
                 'cartservice_cpu_usage': int("{}".format(obs[7])),
                 'cartservice_mem_usage': int("{}".format(obs[8])),
                 ...
                 'adservice_num_pods': int("{}".format(obs[9])),
                 #       'adservice_desired_replicas': int("{}".format(obs[7])),
                 'adservice_cpu_usage': int("{}".format(obs[10])),
                 'adservice_mem_usage': int("{}".format(obs[11])),
                 #         'adservice_traffic_in': int("{}".format(obs[22])),
                 #        'adservice_traffic_out': int("{}".format(obs[23])),
                 #       'adservice_latency': float("{:.3f}".format(latency)),

                 'paymentservice_num_pods': int("{}".format(obs[12])),
                 #       'paymentservice_desired_replicas': int("{}".format(obs[9])),
                 'paymentservice_cpu_usage': int("{}".format(obs[13])),
                 'paymentservice_mem_usage': int("{}".format(obs[14])),
                 ...
                 'shippingservice_num_pods': int("{}".format(obs[15])),
                 #       'shippingservice_desired_replicas': int("{}".format(obs[1])),
                 'shippingservice_cpu_usage': int("{}".format(obs[16])),
                 'shippingservice_mem_usage': int("{}".format(obs[17])),
                 ...
                 'currencyservice_num_pods': int("{}".format(obs[18])),
                 #          'currencyservice_desired_replicas': int("{}".format(obs[37])),
                 'currencyservice_cpu_usage': int("{}".format(obs[19])),
                 'currencyservice_mem_usage': int("{}".format(obs[20])),
                 ...
                 'redis-cart_num_pods': int("{}".format(obs[21])),
                 #          'redis-cart_desired_replicas': int("{}".format(obs[43])),
                 'redis-cart_cpu_usage': int("{}".format(obs[22])),
                 'redis-cart_mem_usage': int("{}".format(obs[23])),
                 ...
                 'checkoutservice_num_pods': int("{}".format(obs[24])),
                 #          'checkoutservice_desired_replicas': int("{}".format(obs[49])),
                 'checkoutservice_cpu_usage': int("{}".format(obs[25])),
                 'checkoutservice_mem_usage': int("{}".format(obs[26])),
                 ...
                 'frontend_num_pods': int("{}".format(obs[27])),
                 #          'frontend_desired_replicas': int("{}".format(obs[55])),
                 'frontend_cpu_usage': int("{}".format(obs[28])),
                 'frontend_mem_usage': int("{}".format(obs[29])),
                 #            'frontend_traffic_in': int("{}".format(obs[58])),
                 #           'frontend_traffic_out': int("{}".format(obs[59])),
                 #          'frontend_latency': float("{:.3f}".format(latency)),

                 'emailservice_num_pods': int("{}".format(obs[30])),
                 #      'emailservice_desired_replicas': int("{}".format(obs[61])),
                 'emailservice_cpu_usage': int("{}".format(obs[31])),
                 'emailservice_mem_usage': int("{}".format(obs[32])),
